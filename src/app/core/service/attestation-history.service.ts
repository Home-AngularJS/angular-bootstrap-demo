...
export class AttestationHistoryService {
  ...
  async query(tableState: TableState) {
    ...
    this.dataSource = new AttestationHistoryDataSource(this.rest);
  ...
  resetBtnFilters(filter: any, tableState: TableState) {
    if (filter.deviceSn==='' && filter.terminalId==='' && filter.deviceName==='' && filter.attestationPhase==='' && filter.date==='' && filter.startDate==='' && filter.endDate===''
  ...
  private setBtnFilter(filter: any, btnFilter: any) {
    ...
    if (btnFilter.field==='startDate') filter.startDate = btnFilter.value;
    if (btnFilter.field==='endDate') filter.endDate = btnFilter.value;
